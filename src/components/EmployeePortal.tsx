...
import { 
  Camera, MapPin, Upload, FileText, CalendarCheck, 
  DollarSign, CheckCircle2, UserCheck, ShieldAlert, RefreshCw,
...
interface EmployeePortalProps {
  ...
  onClockIn: (logUrl: string, lat: number, lng: number, customDate?: string, customTime?: string) => Promise<void> | void;
...
export default function EmployeePortal({
  ...
  // Wrapper to sync employee updates to Firebase
  const handleUpdateEmployeeWithFirebase = async (updated: Employee) => {
    try {
      // Sync to Firebase
  ...
  const processFileUpload = (file: File, key: string, label: string) => {
    ...
    reader.onload = () => {
      ...
      handleUpdateEmployeeWithFirebase({
        ...employee,
        [key]: 'uploaded',
  ...
  const handleDownloadPayslip = async (payslip: Payslip) => {
    try {
      await generatePayslipPDF(payslip, employee, payslipFormat);
      toast("✓ Payslip downloaded successfully!", "success");
    } catch (error) {
      console.error('Error downloading payslip:', error);
      toast("Failed to download payslip. Please try again.", "error");
    }
  };
    ...
    <div className="space-y-6 text-left relative">
      ...
      <div className="flex bg-slate-100/60 dark:bg-slate-950 p-1 rounded-2xl text-xs font-bold border border-slate-202 dark:border-slate-850 select-none">
        {[
          ...
          { key: 'documents', label: 'Documents', icon: <Upload className="w-4 h-4 text-indigo-500" /> },
          { key: 'leaves', label: 'Leaves Balance', icon: <CalendarCheck className="w-4 h-4 text-emerald-500" /> },
          { key: 'payslips', label: 'Payslips', icon: <DollarSign className="w-4 h-4 text-amber-500" /> }
        ].map(seg => (
          <button
            key={seg.key}
      ...
      <div className="bg-white/50 dark:bg-slate-900/10 border border-slate-200/50 dark:border-slate-800/80 rounded-3xl p-6 sm:p-8 backdrop-blur shadow-2xs">
          ...
          <div className="space-y-6">
            <div className="pb-3 border-b border-dashed border-slate-200 dark:border-slate-800 text-left">
              ...
              <p className="text-xs text-slate-500 dark:text-slate-405 mt-0.5">
                Confirm your physical presence. Attendance records require live GPS coordinates.
              </p>
            </div>
            ...
            {checkedInToday ? (
              <div className="p-3.5 bg-emerald-500/10 border border-emerald-500/20 text-emerald-600 dark:text-emerald-400 rounded-2xl flex items-center gap-3 text-xs font-semibold select-none text-left">
                ...
                <div className="space-y-0.5">
                  ...
                  <span className="text-[10.5px] text-emerald-600/80 font-mono block">Your presence has been certified securely.</span>
                </div>
              </div>
            ) : (
              <div className="p-3.5 bg-amber-500/10 border border-amber-500/20 text-amber-600 dark:text-amber-400 rounded-2xl flex items-center gap-3 text-xs font-semibold select-none text-left">
                ...
                <div className="space-y-0.5">
                  <span className="font-extrabold block">⚠️ Attendance Entry Outstanding for Today</span>
            ...
            {!useManualForm ? (
                ...
                <div className="space-y-6">
                  <div className="p-5 rounded-2xl bg-indigo-50/20 dark:bg-slate-900/30 border border-indigo-200/20 dark:border-slate-800 space-y-4">
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-black uppercase text-indigo-755 dark:text-sky-450 tracking-wider font-display">📍 Location Telemetry</span>
                      <button onClick={fetchGeolocation} className="text-[10px] font-bold text-indigo-600 hover:text-indigo-800 dark:text-sky-400 hover:underline">{gpsLoading ? "Polling GPS..." : "Refresh GPS"}</button>
                    </div>

                    {gpsLoading ? (
                      <div className="flex items-center gap-2.5 text-xs text-slate-500 font-mono animate-pulse">
                        <RefreshCw className="w-3.5 h-3.5 animate-spin text-slate-400" />
                        <span>Pinging satellite beacons...</span>
                      </div>
                    ) : gpsError ? (
                      ...
                      <div className="space-y-3 font-mono text-xs">
                        <div className="flex justify-between items-center py-1.5 border-b border-slate-200/50 dark:border-slate-800/40">
                          <span className="text-slate-455">Latitude:</span>
                          <span className="font-extrabold text-slate-805 dark:text-white select-all">{(gpsCoords?.lat || 17.4772).toFixed(6)}° N</span>
                        </div>
                        <div className="flex justify-between items-center py-1.5 border-b border-slate-200/50 dark:border-slate-800/40">
                          <span className="text-slate-455">Longitude:</span>
                          <span className="font-extrabold text-slate-805 dark:text-white select-all">{(gpsCoords?.lng || 78.5711).toFixed(6)}° E</span>
                        </div>
                        <div className="flex justify-between items-center py-1">
                          ...
                          <span className="font-extrabold text-emerald-600 dark:text-emerald-450 flex items-center gap-1">
                            ...
                            {gpsCoords ? 'GPS Locked' : 'No Signal'}
                          </span>
                        </div>
                      </div>
                    )}
                  ...
                  <div className="p-4 rounded-xl bg-slate-50 dark:bg-slate-950/20 text-xs text-slate-455 dark:text-slate-500 leading-relaxed font-sans">
                    📸 <strong className="font-bold text-slate-700 dark:text-slate-350">Selfie & GPS Notice:</strong> Your selfie and coordinates are encrypted and used solely for payroll audits.
                  </div>
                ...
                <div className="flex justify-center flex-col items-center space-y-2">
                  ...
                  {capturedSelfie && (
                    <div className="text-center">
                      <img src={capturedSelfie} alt="Selfie" className="w-24 h-24 rounded-full object-cover border-2 border-emerald-500 mx-auto" />
                      <p className="text-[10.5px] text-emerald-600 dark:text-emerald-400 font-black tracking-wide bg-emerald-500/10 px-2.5 py-1 rounded-full border border-emerald-500/20 mt-2">
                        ✓ Selfie captured
                      </p>
                    </div>
                  )}
              ...
              <div className="max-w-xl p-6 border border-slate-200 dark:border-slate-800 rounded-3xl bg-slate-50/50 dark:bg-slate-950/25 space-y-5 animate-fade-in text-xs font-semibold">
                <div className="space-y-1 text-left">
                  <h5 className="text-xs font-black uppercase text-indigo-700 dark:text-indigo-400">Manual Attendance Log</h5>
                  <p className="text-[11px] text-slate-455">Enter your log details and coordinates manually.</p>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-1 text-left">
                    <label className="block text-slate-455 leading-none">Time *</label>
                    <input
                      type="text"
                      required
                  ...
                  <div className="space-y-1 text-left">
                    <label className="block text-slate-455 leading-none">Latitude *</label>
                    <input
                      type="text"
                      required
                  ...
                  <div className="space-y-1 text-left">
                    <label className="block text-slate-455 leading-none">Longitude *</label>
                    <input
                      type="text"
                      required
            ...
            <div className="border-t border-slate-100 dark:border-slate-800/80 pt-6 mt-6 text-left">
              <h5 className="text-xs font-black uppercase tracking-wider text-slate-800 dark:text-slate-200 mb-3 flex items-center gap-1.5 font-display">
                ...
                <span>Your Attendance History ({myLogs.length})</span>
              </h5>
              ...
              {myLogs.length === 0 ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {myLogs.map(log => (
                    <div key={log.id} className="p-3.5 rounded-xl border border-slate-150 dark:border-slate-800 bg-white dark:bg-slate-900/10 flex flex-col justify-between shadow-3xs hover:border-indigo-300 dark:hover:border-sky-500 transition">
                      <div className="flex justify-between items-start">
                        <div className="space-y-0.5">
                          <span className="text-[9px] bg-emerald-500/10 text-emerald-600 px-1.5 py-0.5 rounded-md font-bold font-mono">
                            {log.isManualOverride ? "📝 MANUAL" : "⚡ GPS"}
                          </span>
                      ...
                      <div className="text-[10px] font-mono text-slate-400 border-t border-dashed border-slate-100 dark:border-slate-800/50 pt-1.5 mt-1.5">
                        📍 GPS: <span className="font-bold text-slate-600 dark:text-slate-200">{(log.latitude || 17.4772).toFixed(4)}° N, {(log.longitude || 78.5711).toFixed(4)}° E</span>
                        {log.selfieUrl && (
                          <div className="mt-1">
                            <img src={log.selfieUrl} alt="Selfie" className="w-10 h-10 rounded-full object-cover border border-slate-200" />
                          </div>
                        )}
                      </div>
                    </div>
        ...
        {/* TAB 2: Documents - Keep existing code */}
          ...
          <div className="space-y-8 text-left">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 pb-3 border-b border-dashed border-slate-200 dark:border-slate-800">
              <div>
                ...
                <p className="text-xs text-slate-500 dark:text-slate-405 mt-0.5">
                  Maintain your digital identity files.
                </p>
              </div>
            ...
            {isAddingCustomDoc && (
              <form onSubmit={handleCreateCustomDocSlot} className="p-4 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-950/20 max-w-md space-y-3 animate-fade-in">
                <h5 className="text-xs font-black uppercase text-indigo-700 dark:text-sky-400">Custom Document</h5>
                <div className="space-y-2">
                  <input
                    type="text"
                    required
                    placeholder="e.g. GST Registration, Driving License"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
              {[...docUploads.map(d => ({ key: d.key, label: d.label, help: d.help, isStandard: true })), 
                ...(employee.uploadedFilesList || [])
                  ...
                  .map(f => ({ key: f.key, label: f.label, help: 'Added manually', isStandard: false }))
              ].map(doc => {
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="mt-2.5 flex items-center gap-1.5 select-none leading-none">
                        ...
                        <span className={`text-[9.5px] font-bold uppercase tracking-wider ${isVerified ? "text-emerald-600 dark:text-emerald-450" : isUploaded ? "text-amber-550 dark:text-amber-500" : "text-slate-400"}`}>
                          {isVerified ? "Verified" : isUploaded ? "Pending" : "Awaiting"}
                        </span>
                      </div>
                    </div>
            ...
            <div className="mt-12 pt-8 border-t border-dashed border-slate-200 dark:border-slate-800 text-left">
              <div className="flex items-center justify-between pb-3">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <h5 className="text-xs font-black uppercase tracking-wider text-slate-800 dark:text-slate-150">Recycle Bin</h5>
                    <p className="text-[10px] text-slate-400">Restore or permanently delete files.</p>
                  </div>
                </div>
                <span className="text-[10px] px-2.5 py-0.5 rounded bg-slate-100 dark:bg-slate-800 text-slate-450 font-mono font-bold">
                  {employeeTrash.length} Items
                </span>
              </div>
              ...
              {employeeTrash.length === 0 ? (
                <div className="py-8 bg-slate-50/20 dark:bg-slate-950/20 border border-slate-150 dark:border-slate-850 rounded-2xl text-center">
                  <span className="text-xs text-slate-400 italic">Recycle bin is empty.</span>
                </div>
              ) : (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        {activeSegment === 'leaves' && (
          <div className="space-y-6">
            <div className="pb-3 border-b border-dashed border-slate-205 dark:border-slate-850 text-left">
              <h4 className="text-sm font-bold text-slate-801 dark:text-slate-200 uppercase tracking-wider">Leave Balance</h4>
              <p className="text-xs text-slate-500 dark:text-slate-405 mt-0.5">Your allocated and remaining leaves for FY26.</p>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 text-left">
              {[
                ...
                { label: 'Annual Leave (AL)', max: 15, used: 15 - employee.leaveBalance.annual, rem: employee.leaveBalance.annual, color: 'from-emerald-400 to-teal-500' }
              ].map(leave => {
        ...
        {activeSegment === 'payslips' && (
          <div className="space-y-6">
            <div className="pb-3 border-b border-dashed border-slate-205 dark:border-slate-850 text-left">
              <h4 className="text-sm font-bold text-slate-800 dark:text-white uppercase tracking-wider">Payslips</h4>
              <p className="text-xs text-slate-455 mt-0.5">Your official payroll cards.</p>
            </div>
            ...
            {employeePayslips.length === 0 ? (
              <div className="p-12 border border-dashed border-slate-200 dark:border-slate-800 rounded-2xl text-center bg-white dark:bg-slate-950/20">
                ...
                <p className="text-xs text-slate-500 font-semibold">No payslips available.</p>
              </div>
            ) : (
                  ...
                  <div key={pay.id} className="p-5 rounded-2xl border border-emerald-500/10 bg-emerald-500/5 dark:bg-emerald-500/2 flex flex-col justify-between h-[210px] shadow-sm hover:shadow-xs transition">
                    <div className="space-y-4">
                      <div className="flex justify-between items-baseline font-mono select-none">
                        <span className="text-[10px] text-slate-455 uppercase font-black">ID: {pay.id}</span>
                        <span className="text-emerald-600 dark:text-emerald-450 font-black text-xs uppercase">PAID</span>
                      </div>

                      <div className="flex justify-between items-center text-left">
                        <div>
                          <span className="text-xs text-slate-450 uppercase font-mono block">Month</span>
                          <span className="text-sm font-black text-slate-805 dark:text-white font-display mt-0.5 block">{pay.monthYear}</span>
                        </div>

                        <div className="text-right">
                          <span className="text-xs text-slate-455 block">Net Salary</span>
                    ...
                    <div className="border-t border-slate-200/40 dark:border-slate-850/60 pt-3 flex items-center justify-between gap-4 text-xs font-semibold">
                      <span className="text-[9px] text-slate-400 font-mono">Issued: {pay.deliveredAt}</span>
                      <button
