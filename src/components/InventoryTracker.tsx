...
export default function InventoryTracker({ items, onUpdateItems, toast }: InventoryTrackerProps) {
  ...
  const handleCreateItem = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast('Failed to add inventory item. Please try again.', 'error');
    }
  };

  const handleUpdateStock = async (itemId: string, increment: boolean, changeAmount = 1) => {
    try {
      const updated = items.map(item => {
        if (item.id === itemId) {
          ...
          inventoryService.updateItem(itemId, updatedItem).catch(err => 
      ...
      toast('Failed to update stock level. Please try again.', 'error');
    }
  };
